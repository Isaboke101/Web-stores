  ...
  if ($post['title'] === '' || $post['body'] === '') {
    ...
    if ($post['status'] === 'published' && $notify) {
      ...
      foreach ($subs as $email) {
        ...
        $bodyHtml = '<p style="font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#C5A059;margin:0 0 10px">New Article</p>'
                  . '<p><strong>' . esc($post['title']) . '</strong></p>'
        ...
        if (send_mail($email, $subject, $bodyTxt,
              email_template($post['title'], $bodyHtml, 'Read the Article', $url, $footerNote))) $sent++;
      }
